                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="price-main" name="sub_products[0][price]" type="number" step="0.01" min="0"
                                    class="w-full rounded-md border border-gray-300 p-2"
                                    placeholder="Enter price" />
                            </div>
                            <div class="space-y-2">
                                <label for="basePrice-main" class="block text-sm font-medium">Base Price</label>
                                <input id="basePrice-main" name="sub_products[0][base_price]" type="number" step="0.01" min="0"
                                    class="w-full rounded-md border border-gray-300 p-2"
                                    placeholder="Enter base price" />
                            </div>
                            <div class="space-y-2">
                                <label for="quantity-main" class="block text-sm font-medium">Quantity</label>
                                <input id="quantity-main" name="sub_products[0][quantity]" type="number" min="0" value="0"
                                    class="w-full rounded-md border border-gray-300 p-2"
                                    placeholder="Enter quantity" />
                            </div>
                            <div class="space-y-2">
                                <label for="subProductStatus-main" class="block text-sm font-medium">Status</label>
                        ...
                        <div class="space-y-4">
                            <div class="space-y-2">
                                ...
                                <input id="hiddenImageInput-main" name="sub_products[0][images][]" type="file"
    ...
    $(document).ready(function() {
        ...
        function createNewSubProductComponent() {
            ...
            return `
                <div class="sub-product-component mt-6 border-t pt-6" data-sub-product-id="${subProductId}">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold">Product Variant</h3>
                        <button type="button" class="remove-sub-product border-red-500 cursor-pointer border p-1 rounded-full">
                            <i class="fa-solid flex items-center justify-center h-5 w-5 fa-minus text-red-500 rounded-full"></i>
                        </button>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div class="space-y-4">
        ...
        $(document).on('click', '.add-sub-product', function() {
            ...
            if (!imageArrays[subProductId] || imageArrays[subProductId].length === 0) {
                $(`#imagesContainer-${subProductId}`).after('<div class="text-red-500 text-sm mt-1">At least one image is required</div>');
                isValid = false;
            }

            if (!isValid) {
                showToast('Please fill in all required fields and upload at least one image before adding a new variant.', 'error');
                return;
            }
        ...
        $(document).on('click', '.addSpec', function() {
            ...
            const subProductIndex = $('#sub-products-forms-container').children().index($(`[data-sub-product-id="${subProductId}"]`));
        ...
        $('#step-2').validate({
            ...
            submitHandler: function(form) {
                ...
                const url = '{{ route('admin.products.store-step-2', ['vendor_id' => ':vendor']) }}'.replace(':vendor', '{{ $productId }}');
                $.ajax({
                    url: url,
                    type: 'POST',
                    headers: {
        ...
        // Initialize validation for the main sub-product component
        initializeComponentValidation($('#sub-products-forms-container').find('.sub-product-component').first());
        ...
        function validateAllSubProductForms() {
            ...
            $components.each(function(index) {
                ...
                if (!imageArrays[subProductId] || imageArrays[subProductId].length === 0) {
                    console.log(imageArrays);
                        $(`#imagesContainer-${subProductId}`).after(`<div id="imageError-${subProductId}" class="text-red-500 text-sm mt-1">At least one image is required</div>`);
